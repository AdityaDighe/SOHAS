...
public class PatientDAOImpl implements PatientDAO {
	...
	public List<Doctor> getDoctors(String location, Time time, Date date) {
		...
		subquery.select(appointmentRoot.get("doctor").get("doctorId")).where(
				cb.equal(appointmentRoot.get("date"), date), cb.equal(appointmentRoot.get("time"), time),
				cb.equal(appointmentRoot.get("status"), "BOOKED"));

		// Main Query :
		cq.select(doctorRoot)
				.where(cb.and(cb.equal(doctorRoot.get("city"), location),
	...
	public List<Appointment> getAppointment(int id) {
		...
		return sessionFactory.getCurrentSession().createQuery(hql, Appointment.class).setParameter("patientId", id)
				.list();
	}
